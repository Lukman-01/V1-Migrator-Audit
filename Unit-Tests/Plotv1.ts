import {
  ...
  describe("ATLPLOT", function () {
    async function deployATLPLOTFixture() {
      ...
      const nft = await ATLPLOT.deploy(mockToken);
    ...
    describe("Deployment", function () {
      it("Should set the right owner", async function () {
        const { nft, owner } = await loadFixture(deployATLPLOTFixture);
        expect(await nft.owner()).to.equal(owner.address);
      });
  
      it("Should set the correct payment token", async function () {
        const { nft, mockToken } = await loadFixture(deployATLPLOTFixture);
      ...
      it("Should have correct initial values", async function () {
        const { nft } = await loadFixture(deployATLPLOTFixture);
    ...
    describe("Batch Management", function () {
      it("Should create a new batch", async function () {
        const { nft, BATCH_QUANTITY, BATCH_PRICE } = await loadFixture(deployATLPLOTFixture);
    ...
    describe("Fee Management", function () {
      ...
      //   const { nft, TX_FEE } = await loadFixture(deployATLPLOTFixture);
      ...
      it("Should update fee collector", async function () {
        const { nft, feeCollector } = await loadFixture(deployATLPLOTFixture);
    ...
    describe("Participant Management", function () {
      it("Should set free participant controller", async function () {
        const { nft, addr1 } = await loadFixture(deployATLPLOTFixture);
